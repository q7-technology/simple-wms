...
| First ERP | An SAP adapter | Built. An SAP subscriber is an ordinary row in `subscriber` with `transport: "sap_rfc"`, on the same queue and the same backoff, and each event becomes one `BAPI_GOODSMVT_CREATE`. `pyrfc` is an optional extra, because the SAP RFC SDK is licensed; everything but opening the connection runs and is tested without it. The signed outbound events and the REST inbound door are unchanged and serve everyone else. |
